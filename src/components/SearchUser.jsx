  ...
  render() {
    ...
    if(data.loading) {
      ...
      return (
        ...
        <div className={this.props.search === 'users' ? "search__user__grid fade-in-out" : "search__user__grid fade-in"}>
          {!data.loading ? (
              ...
              .map((item, index) => (
                ...
                >
                        ...
                        <ul className="list__inline list__border">
                          {item.skills.length > 0 ? (
                            item.skills.map(item => (
                              <li key={item.title + '-' + item.id || item.first_name + '-' + item.id}>
                                {item.name}
                              </li>
                            ))
                          ) : (
                  ...
                  <div className="search__desc">
                    {item.bio
                      ? item.bio
                      : this.props.search === 'users' ? 'This user does not have any self introductions.' : ''}
                      {item.description
                      ? item.description
                      : this.props.search === 'projects' ? 'This project still does not have any introductions.' : ''}
                  </div>
                </div>
              ))
          ) : (
