    ...
    if(this.props.search === 'users') {
      return (
        ...
        {!data.loading ? (data[this.props.search] && 
          ...
          .map((item, index) => (
            <div className="search__user d-flex flex-column" key={this.props.search + '-' + index}>
              ...
              {item.bio ? item.bio : 'This user does not have any self introductions.'}
              </div>
            </div>))) : (
        <div>Loading...</div>
        )}
      ...
      return (
        ...
        {!data.loading ? (data[this.props.search] && 
          ...
          .map((item, index) => (
            <div className="search__user d-flex flex-column" key={this.props.search + '-' + index}>
                <div className="d-flex flex-row align-items-center justify-content-center">
                    <span className="search__thumbnail" >{item.title.slice(0,1) }</span>
                  <div className="search__title__grid">
